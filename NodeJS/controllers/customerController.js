...
router.get('/', (req, res) => {
    Customer.find((err, docs) => {
        ...
        else { console.log('Error in retrieving customers : ' + JSON.stringify(err, undefined, 2)); }
    });
});
...
router.get('/:id', (req, res) => {
    ...
    Customer.findById(req.params.id, (err, doc) =>{
        if(!err) { res.send(doc); }
        else { console.log('Error in retrieving customer : ' + JSON.stringify(err, undefined, 2)); }
    });
});
